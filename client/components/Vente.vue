            ...
            <form @submit.prevent="createPost()" id="mon_form">
                <div class="ajouter_produit">
                    ...
                    <div class="field_ville">
                        ...
                        <select id="ville" name="ville" v-model="ville">
                            <option value="none" selected disabled hidden>ville</option>
                            <option v-for="ville in villes" :key="ville.id_ville" :value="ville.id_ville">{{ville.nom}}</option>
                        </select>
                    </div>
...
<style scoped>
    ...
    .field_disponibilité, .field_label, .field_prix, .field_quantité, .field_photo, .field_produit, .field_adresse, .field_ville{
        margin-bottom: .9em;
    }
    .produits_en_vente{
        display: flex;
...
<script>
    module.exports = {
        ...
        async created(){
            ...
            const result3 = await axios.get('/api/villes', {})
            this.villes = result3.data
        },
        methods: {
            async createPost() {
